...
function [Tout,Error,Xout,Xdout,uout,EulerDistanceHistory] = EulerOdeIntegrator(xIn,OMEGA_fn)
...
EulerDistanceHistory = [];
    ...
    %     [t,x] = ode45(@flow,tspan,xIn,options);
        ...
        R_euler= eulerToSO3(x(1),x(2),x(3));
        EulerSO3dNorm = NormalizedEuclideanDistance(R_euler);
        
        % plots and output signals
        e       = xref - x;        
        EulerDistanceHistory=[EulerDistanceHistory EulerSO3dNorm];
        Error   = [Error e];
